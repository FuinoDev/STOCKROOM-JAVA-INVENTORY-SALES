...
public final class OverviewPanel extends AppPanel {
    private final JPanel content=new DashboardContent();private boolean loading;
    public OverviewPanel(DashboardFrame frame) {
        ...
        JScrollPane scroll=Ui.scroll(content);scroll.setBorder(null);scroll.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
        scroll.getViewport().setBackground(Ui.BG);add(scroll);content.add(Ui.label("Loading your workspace…",15,Ui.MUTED,false));
    }
    ...
    private void render(ReportService.Overview overview) {
        ...
        if(overview.alerts().isEmpty())alerts.add(Ui.label("All stocked up. No low-stock products right now.",15,Ui.GREEN,false));
        else {
            int visibleRows=Math.min(5,Math.max(3,table.getRowCount()));
            table.setPreferredScrollableViewportSize(new Dimension(700,visibleRows*table.getRowHeight()));
            JScrollPane tableScroll=Ui.scroll(table);tableScroll.setColumnHeaderView(table.getTableHeader());alerts.add(tableScroll);
        }
        addCard(alerts);content.revalidate();content.repaint();
    }
    /** Each card keeps its useful height; small windows scroll the page instead of crushing tables. */
    private void addCard(JPanel card) {
        if(content.getComponentCount()>0)content.add(Box.createVerticalStrut(22));
        JPanel row=new JPanel(new BorderLayout()) {
            @Override public Dimension getMinimumSize() {return new Dimension(0,getPreferredSize().height);}
            @Override public Dimension getMaximumSize() {return new Dimension(Integer.MAX_VALUE,getPreferredSize().height);}
        };
        row.setOpaque(false);row.setAlignmentX(Component.LEFT_ALIGNMENT);row.add(card);content.add(row);
    }
    private static final class DashboardContent extends JPanel implements Scrollable {
        DashboardContent() {setLayout(new BoxLayout(this,BoxLayout.Y_AXIS));setOpaque(false);}
